...
- Enable Long File Paths. The Python installer may have done this or it can be enabled by modifying the Windows Registry (must be done as an administrator). Change the registry key `HKEY_LOCAL_MACHINE/System/CurrentControlSet/Control/FileSystem/LongPathsEnabled` to `1`.
...
- `DEPENDENCIES_BUILD_DIR=<path to build>`
