{
	...
	"commands" : [
		...
		"./b2 tools/bcp",
		"./bjam -d+2 -j {jobs} --disable-icu cxxflags='-std=c++11' variant={boostBuildType} link=shared threading=multi install",
	...
	"platform:windows" : {
		...
		"commands" : [
			...
			"b2 -d+2 --prefix={buildDir} --toolset={boostMsvcVersion} architecture=x86 address-model=64 --build-type=complete variant={boostBuildType} link=shared threading=multi -s ZLIB_SOURCE=%ROOT_DIR%\\Zlib\\working\\zlib-1.2.11 -s ZLIB_INCLUDE={buildDir}\\include -s ZLIB_LIBPATH={buildDir}\\lib -s ZLIB_BINARY=zlib install"
