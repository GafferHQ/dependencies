{
	...
	"platform:windows" : {
		...
		"commands" : [
			...
			"echo \"using python ^: 2.7 : {buildDir} ^: {buildDir}\\include ^: {buildDir}\\lib ^: ^<address-model^>64 ^; >> project-config.jam",
			"b2 --prefix={buildDir} --toolset={boostMsvcVersion} architecture=x86 address-model=64 --build-type=complete variant=release link=shared threading=multi -s ZLIB_SOURCE=%ROOT_DIR%\\Zlib\\working\\zlib-1.2.11 -s ZLIB_INCLUDE={buildDir}\\include -s ZLIB_LIBPATH={buildDir}\\lib -s ZLIB_BINARY=zlib install"
